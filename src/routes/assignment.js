...
routes.get("/:id_team/user/:id", verifyUserOnTeam, AssignmentController.getTeamAssignments);//receber as tarefas de uma equipe em especifico, ok
routes.put(
  "/:id",
  (req, res, next) => {
    res.locals.permission = teamPermissions.moderator;
...
routes.get("/@me/team/:id_team/", AssignmentController.me, verifyUserOnTeam, AssignmentController.getTeamAssignments);//receber as tarefas de uma equipe em especifico, ok
routes.post(
  "/@me/complete-assignment/:id_assignment",
  AssignmentController.me,
  async (req, res, next)=>{
    try {
      const {index, assignment} = await UserAssignmentRepository.verifyUserHasAssignment(req.params.id_assignment, res.locals.user.id)
